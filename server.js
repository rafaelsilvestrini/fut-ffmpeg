...
const TRANSFERMARKT_LATEST_TRANSFERS_URL = 'https://www.transfermarkt.com/transfers/neuestetransfers/statistik/plus/?plus=0&galerie=0&wettbewerb_id=alle&verein_land_id=&selectedOptionInternalType=top15&land_id=&minMarktwert=0&maxMarktwert=500.000.000&minAbloese=0&maxAbloese=500.000.000&yt0=Show';
...
const parseLatestTransfersHtml = (htmlContent, limit = 25) => {
    ...
    const parsePlayer = (cell) => {
        const playerLink = cell.find('td.hauptlink a[href*="/profil/spieler/"]').first();
        ...
        return {
            ...
            id: playerUrl,
            url: playerUrl,
            position: cleanText(cell.find('table.inline-table tr').eq(1).find('td').last().text()),
            image_url: imageUrl(cell.find('img').first())
        };
    };
    const parseClub = (cell) => {
        const clubLink = cell.find('td.hauptlink a[href*="/startseite/verein/"]').first();
        const fallbackClubLink = cell.find('a[href*="/startseite/verein/"]').first();
        ...
        return {
            name: cleanText(selectedClubLink.text()) || cleanText(clubImg.attr('alt')),
            full_name: cleanText(selectedClubLink.attr('title')) || cleanText(clubImg.attr('title')),
    ...
    return $('table.items > tbody > tr')
        ...
        .map((_, row) => {
            ...
            const nationalities = cells.eq(2).find('img').map((__, img) => (
                cleanText($(img).attr('title') || $(img).attr('alt'))
            )).get().filter(Boolean);
...
const formatMarketValue = (val, lang = 'pt') => {
    ...
    if (num >= 1000000) {
        ...
        formattedNum = Number.isInteger(valM) ? valM.toString() : valM.toFixed(1).replace('.', ',');
        suffix = 'm';
    } else if (num >= 1000) {
        let valK = num / 1000;
        formattedNum = Number.isInteger(valK) ? valK.toString() : valK.toFixed(1).replace('.', ',');
        suffix = 'k';
    } else {
...
const convertImageToMp4 = (imageBuffer) => new Promise((resolve, reject) => {
    const requestTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'transfer-video-'));
    ...
    command
        ...
        .outputOptions([
            ...
            '-t', `${duration}` // Corta exatamente na duração especificada
        ]);
...
app.post('/api/generate-transfer-card', async (req, res) => {
    try {
        ...
        const [bgB64, playerB64, fromB64, toB64, handshakeB64, logoB64] = await Promise.all([
